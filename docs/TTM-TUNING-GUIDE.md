...
### Method 2: systemd-boot (SteamOS, Modern Arch)

**Supported distros:** SteamOS 3.x, some Arch installations, modern systemd-based systems

1. List boot entries to find your active entry:
   ```bash
   ls /boot/loader/entries/
   ```

2. Edit the appropriate entry (example: `steamdeck.conf` or `arch.conf`):
   ```bash
   sudo nano /boot/loader/entries/steamdeck.conf
   ```

3. Add TTM parameters to the `options` line:
   ```
   title   SteamOS
   linux   /vmlinuz-linux
   initrd  /initramfs-linux.img
   options root=PARTUUID=xxxx rw ttm.pages_limit=4194304 ttm.page_pool_size=4194304
   ```
...
### Method 3: Fedora with grubby (Jeff Geerling's Method)
