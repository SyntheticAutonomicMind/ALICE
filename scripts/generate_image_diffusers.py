...
try:
    ...
    # CRITICAL: Disable problematic SDPA backends for AMD GPU (gfx1103) compatibility
    # Flash and memory-efficient attention cause GPU hangs on ROCm/AMD
    if torch.cuda.is_available():
        try:
            torch.backends.cuda.enable_flash_sdp(False)
            torch.backends.cuda.enable_mem_efficient_sdp(False)
            torch.backends.cuda.enable_math_sdp(True)
            print("AMD GPU fix: Disabled flash/mem_efficient SDPA, enabled math-only")
...
SCHEDULERS = {
    'dpm++': {
        ...
        'config': {'use_karras_sigmas': False, 'algorithm_type': 'dpmsolver++'}
    },
    'dpm++_karras': {
        'class': DPMSolverMultistepScheduler,
    ...
    'dpm++_sde': {
        'class': DPMSolverMultistepScheduler,
        # solver_order=1 avoids second-order update that causes IndexError on final step
        'config': {'use_karras_sigmas': False, 'algorithm_type': 'sde-dpmsolver++', 'solver_order': 1}
    },
    'dpm++_sde_karras': {
        'class': DPMSolverMultistepScheduler,
        # solver_order=1 avoids second-order update that causes IndexError on final step
        'config': {'use_karras_sigmas': True, 'algorithm_type': 'sde-dpmsolver++', 'solver_order': 1}
    },
    'euler': {
...
def generate_image(
    ...
    try:
        ...
        if device == "mps":
            ...
            # CRITICAL: AMD GPUs (gfx1103/Phoenix) have instability with bfloat16
            # Force float16 for better compatibility on ROCm
            dtype = torch.float16
            print("NOTE: Using float16 on CUDA for AMD ROCm compatibility")
        else:
...
Examples:
  ...
  python3 generate_image_diffusers.py \\
    ...
    parser.add_argument(
        ...
        help='Scheduler to use (default: dpm++_sde_karras)'
    )
